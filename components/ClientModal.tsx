...
export function ClientModal({ client, onClose, onSaved }: Props) {
  ...
  return (
    <>
      ...
      {telegramDeepLink && (
        <QRModal data={telegramDeepLink} label="Верификация клиента" subtitle="Сканируйте QR-код. Бот автоматически запросит номер телефона для подтверждения." onClose={() => setTelegramDeepLink('')} />
      )}
