  ...
  return (
    <main className="min-h-screen bg-background mx-auto max-w-4xl px-4 py-10">
      ...
      <section className="mb-12">
        <h2 className="mb-4 text-lg font-semibold">{t("loyalty.tiers")}</h2>
        ...
        >
          <input className="rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" placeholder={t("loyalty.tierForm.namePlaceholder")} value={tierForm.name}
            onChange={(e) => setTierForm({ ...tierForm, name: e.target.value })} required />
          <input className="w-28 rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" type="number" placeholder={t("loyalty.tierForm.minPointsPlaceholder")} value={tierForm.minPoints}
            onChange={(e) => setTierForm({ ...tierForm, minPoints: Number(e.target.value) })} />
          <input className="w-24 rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" type="number" placeholder={t("loyalty.tierForm.discountPlaceholder")} value={tierForm.discount}
          ...
          <button className="rounded bg-foreground px-3 py-1 text-background" type="submit">{t("loyalty.tierForm.add")}</button>
        </form>
      </section>

      <section>
        <h2 className="mb-4 text-lg font-semibold">{t("loyalty.rules")}</h2>
        <ul className="mb-4 flex flex-col gap-2">
          {rules.data?.map((r) => (
        ...
        <form
          ...
          className="flex flex-wrap gap-2"
        >
          <input className="rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" placeholder={t("loyalty.ruleForm.namePlaceholder")} value={ruleForm.name}
          ...
          <select className="rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" value={ruleForm.triggerType}
            ...
            <option value="VISIT">{t("loyalty.ruleForm.triggerType.visit")}</option>
            <option value="PURCHASE">{t("loyalty.ruleForm.triggerType.purchase")}</option>
          </select>
          <input className="w-28 rounded border border-border bg-card px-2 py-1 text-foreground placeholder:text-muted" type="number" placeholder={t("loyalty.ruleForm.pointsPlaceholder")} value={ruleForm.pointsAwarded}
            onChange={(e) => setRuleForm({ ...ruleForm, pointsAwarded: Number(e.target.value) })} />
          <button className="rounded bg-foreground px-3 py-1 text-background" type="submit">{t("loyalty.ruleForm.add")}</button>
        </form>
      </section>
    </main>
  );
